...
// =====================================================================
// PHASE 6 — Persist the verdict to eval/runs/ (workflow scripts can't write files,
// so a single WRITE-SCOPED agent does it). This removes the manual python-extraction
// step the goal-loop depended on. The stamp is generated by the agent (Date.now() is
// banned in scripts). This agent is intentionally EXEMPT from READONLY — its only job
// is to write exactly one JSON file; it must not touch anything else or run git.
phase('Persist verdict')
const REPO = '/Users/xiu/projects/agent-craft/workshop-test/agent-craft-starter'
const runJson = JSON.stringify(result)
const savedPath = await agent(
  `WRITE-SCOPED TASK (this agent is exempt from read-only mode for ONE file only).\n` +
  `Save an eval verdict to disk. Steps, exactly:\n` +
  `1. Get a timestamp: run \`date +%Y-%m-%d_%H%M\` via Bash. Call it STAMP.\n` +
  `2. Build the filename: \`${REPO}/eval/runs/\${STAMP}_${cfg.persona}_${cfg.probe}.json\`\n` +
  `3. Write the JSON below VERBATIM to that path (it is already valid JSON — do not reformat, ` +
  `re-key, or pretty-print in a way that changes content; you may pipe through \`python3 -m json.tool\` ` +
  `for indentation but the data must be byte-faithful).\n` +
  `4. Do NOT run git. Do NOT write, move, or delete any other file. Do NOT modify the skill, ` +
  `the spec, or the curriculum.\n` +
  `5. Return ONLY the absolute path you wrote, nothing else.\n\n` +
  `JSON to write:\n<json>\n${runJson}\n</json>`,
  { label: 'persist-verdict', phase: 'Persist verdict' }
)
log(`saved verdict → ${savedPath}`)
